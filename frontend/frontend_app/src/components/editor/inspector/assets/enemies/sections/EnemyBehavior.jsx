...
export default function EnemyBehavior({ selected, update }) {
  ...
  function setType(nextType) {
    patchBehavior({ behaviorType: nextType });
  }

  function addToOrder(moveId) {
    if (!moveId) return;
    patchBehavior({ cycleOrder: [...cycleOrder, moveId] });
  }

  function removeFromOrder(index) {
    const next = cycleOrder.filter((_, i) => i !== index);
    patchBehavior({ cycleOrder: next });
  }
  ...
  return (
    <InspectorSection title="Behavior" defaultOpen={false}>
