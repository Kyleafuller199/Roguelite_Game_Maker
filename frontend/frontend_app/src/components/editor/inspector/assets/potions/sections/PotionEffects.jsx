 ...
 * PotionEffects.jsx
 *
 * Inspector section for potion.effects[].
 */
...
function makeId(prefix = "eff") { return `${prefix}_${crypto.randomUUID()}`; }
...
export default function PotionEffects({ selected, update }) {
  ...
  function addEffect() {
    setEffects([...effects, normalizeEffectForType(
      { id: makeId("eff"), effectType: "heal", baseValue: 12, target: "self", repeat: 1 },
      "heal"
    )]);
  }
            ...
            return (
              <div key={eff.id} style={{ padding: 12, border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8 }}>
                ...
                {cfg.requiresTarget && (
                  <>
                    <Label>Target</Label>
                    <select value={eff.target ?? "self"} onChange={(e) => patchEffect(eff.id, { target: e.target.value })} style={{ width: "100%", padding: 10, marginBottom: 12 }}>
